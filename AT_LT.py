"""
Standalone PGD adversarial training on long-tailed ImageFolder datasets.

This script is the plain AT-LT baseline. It keeps the standard Madry-style
PGD adversarial training objective and only adapts the data entry to
CIFAR10-LT, CIFAR100-LT, and TinyImageNet-LT ImageFolder datasets. It does
not use TRADES, class-balanced losses, DAFA/CFA/UDR, RobustLT, or CGR-LT.
"""
...
def get_args():
    parser = argparse.ArgumentParser(description="Plain PGD-AT training for long-tailed ImageFolder data")
    ...
    parser.add_argument("--model_dir", default="./model_output/at_lt")
    ...
    parser.add_argument("--lr_schedule", type=str, default="madry",
    ...
    parser.add_argument("--no_random_start", action="store_true", default=False,
                        help="Disable random initialization inside the epsilon ball.")
...
def at_loss(model, x_natural, y, optimizer, args):
    x_adv = pgd_adversary(
        model,
        x_natural,
        y,
...
def train_epoch(args, model, device, optimizer, train_loader, epoch, trainset_size):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss, loss_dict = at_loss(model, data, target, optimizer, args)
        loss.backward()
        optimizer.step()
