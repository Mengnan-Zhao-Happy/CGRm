...
def eval_classwise(model, test_loader, args):
    ...
    print('Clean accuracy: {:.2f}% ({:.0f}/{:.0f})'.format(
        100.0 * clean_acc, cur_nat_correct, total))
    print('PGD robust accuracy: {:.2f}% ({:.0f}/{:.0f})'.format(
        100.0 * pgd_acc, cur_pgd_correct, total))
    print('Clean accuracy classwise: {}'.format(
        np.array2string(clean_acc_classwise * 100.0, precision=2, separator=', ')))
    print('PGD robust accuracy classwise: {}'.format(
        np.array2string(pgd_acc_classwise * 100.0, precision=2, separator=', ')))
